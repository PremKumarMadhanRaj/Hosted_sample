...
window.sfBlazor.Tooltip = function () {
  ...
  function () {
    ...
    SfTooltip.prototype.targetHover = function (e) {
      var proxy = this;
      setTimeout(function () {
        if (!proxy.checkForOpen(proxy.properties.opensOn, proxy.element)) return;
        var target = proxy.properties.target ? sf.base.closest(e.target, proxy.properties.target) : proxy.element;

        if (sf.base.isNullOrUndefined(target) || sf.base.getAttributeOrDefault(target, 'data-tooltip-id', null) !== null) {
          return;
        }

        var targetList = [].slice.call(document.querySelectorAll('[data-tooltip-id= ' + proxy.ctrlId + '_content]'));

        for (var _i = 0, targetList_5 = targetList; _i < targetList_5.length; _i++) {
          var target_1 = targetList_5[_i];
          proxy.restoreElement(target_1);
        }

        proxy.showTooltip(target, proxy.properties.animation.open, e);
      }, this.properties.openDelay, e);
    };
    ...
    SfTooltip.prototype.hideTooltip = function (hideAnimation, e, targetElement) {
      ...
      var proxy = this;
      setTimeout(function () {
        if (_this.checkForOpen(proxy.properties.opensOn, proxy.element)) return;
        var target;

        if (e) {
          target = proxy.properties.target ? targetElement || e.target : proxy.element;
        } else {
          target = document.querySelector('[data-tooltip-id= ' + proxy.ctrlId + '_content]');
        }

        proxy.tooltipEventArgs = {
          type: e ? e.type.toString() : null,
          cancel: false,
          target: proxy.getDomObject('target', target),
          event: e ? e : null,
          element: proxy.getDomObject('tooltipElement', proxy.tooltipEle),
          isInteracted: !sf.base.isNullOrUndefined(e),
          name: 'beforeClose',
          collidedPosition: null
        };
        proxy.beforeCloseTarget = target;
        proxy.beforeCloseAnimation = hideAnimation;
        proxy.isRestrictUpdate = proxy.element.eventList.beforeClose && !proxy.isHidden();
        proxy.element.eventList.beforeClose ? proxy.triggerEvent('TriggerBeforeCloseEvent', proxy.tooltipEventArgs) : proxy.beforeCloseCallBack(false);
      }, this.properties.closeDelay, hideAnimation, e, targetElement);
    };
